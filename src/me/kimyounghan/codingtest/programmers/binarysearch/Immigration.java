...
public class Immigration {
    ...
    long binerySearch(int[] times, int n, long max){
        long left = 1, right = max * n; // left는 입국 심사 시간의 최소값, right는 가장 오래 걸리는 심사관이 모든 사람을 심사할 경우
        long mid = 0; // 이진탐색을 위한 중간값
        long answer = Long.MAX_VALUE;

        while (left <= right) {
            mid = (left + right) / 2;

            if (isPassed(n, times, mid)) { // mid 값으로 모든 사람을 심사할 수 있는지 없는지
